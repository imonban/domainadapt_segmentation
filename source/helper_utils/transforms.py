...
def get_transform(name,conf): 
    ...
    if name =='load': 
        ...
        return LoadImaged(keys=[img_k,lbl_k],reader=NibabelReader) 
    if name =='channel_first': 
        return EnsureChannelFirstd(keys=[img_k,lbl_k])
    ...
    if name =='spacing':
        ...
        return Spacingd(keys=[img_k,lbl_k],pixdim=pix_dim,mode=(img_interp,lbl_interp))
    if name =='rand_crop_label':
        ...
        assert ps_samples <= 1
        return RandCropByPosNegLabeld(keys=[img_k,lbl_k],spatial_size=vox_dim,num_samples=num_samples,label_key=lbl_k,pos=ps_samples,neg=neg_samples,allow_smaller=True)
    if name =='spatial_pad': 
        vox_dim = conf['spacing_vox_dim']
        return SpatialPadd(keys=[img_k,lbl_k], spatial_size=vox_dim)
    ...
    if name == 'rand_flip': 
        ...
        return RandFlipd(keys=['image','label'],prob=prob)
    if name == 'rand_affine': 
        img_interp = conf['spacing_img_interp'] 
        lbl_interp = conf['spacing_lbl_interp']
